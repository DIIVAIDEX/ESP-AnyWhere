...
esp_err_t esp_modem_dce_set_NetMode(esp_modem_dce_t *dce, void *param, void *result)
{
    char command[] = "AT+CNMP=00\r";
    memcpy(command + 8, param, 2); // copy 4 bytes to the "0000" placeholder
    esp_err_t err = esp_modem_dce_generic_command(dce,  command, MODEM_COMMAND_TIMEOUT_DEFAULT, esp_modem_dce_handle_response_default, NULL);
    return err;
}
